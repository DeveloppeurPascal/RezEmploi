...
if (("add" == $op) && $dsp) {
    ...
    $qry = $db->prepare("select * from niveaux_etudes where code=:c");
    ...
    if (empty($libelle)) {
        ...
        if ((false !== ($record = $qry->fetch(PDO::FETCH_OBJ))) && checkVerifChecksum($verif, KEY_VERIF, $record->code, $record->priv_key)) {
            if ($libelle != $record->libelle) {
                $qry = $db->prepare("update niveaux_etudes set libelle=:l where code=:c");
    ...
    $qry->execute(array(":c" => $code));
    if ((false !== ($record = $qry->fetch(PDO::FETCH_OBJ))) && checkVerifChecksum($verif, KEY_VERIF, $record->code, $record->priv_key)) {
        $qry = $db->prepare("delete from niveaux_etudes where code=:c");
        $qry->execute(array(":c" => $code));
        $mode = "lst";
    } else {
        ajoute_erreur("Enregistrement inexistant, suppression impossible.");
...
switch ($mode) {
    case    "add":
        $PageTitle = "Ajout d'un niveau d'études";
        break;
    case    "chg":
        $PageTitle = "Modification du niveau d'études";
        break;
    case    "dlt":
        $PageTitle = "Suppression du niveau d'études";
        break;
    case    "dsp":
        $PageTitle = "Détail du niveau d'études";
        break;
    case    "lst":
        $PageTitle = "Liste des niveaux études";
        break;
    default:
...
?>
    <form action="admin-niveaux-etudes.php" method="post" id="frm">
