...
if (("add" == $op) && $dsp) {
    ...
    if (empty($libelle)) {
        ...
        $qry = $db->prepare("insert into langues (priv_key, libelle) values (:pk, :l)");
    ...
    if (empty($libelle)) {
        ...
        if ((false !== ($record = $qry->fetch(PDO::FETCH_OBJ))) && checkVerifChecksum($verif, KEY_VERIF, $record->code, $record->priv_key)) {
            if ($libelle != $record->libelle) {
                $qry = $db->prepare("update langues set libelle=:l where code=:c");
...
?>
    <form action="admin-langues.php" method="post" id="frm">
        ...
        <section class="fdb-block pt-0">
            <div class="container">
                ...
                <?php if ("lst" == $mode) {
                    $qry = $db->prepare("select * from langues order by libelle");
                    $qry->execute(array());
                    ?>
