...
if (("add" == $op) && $dsp) {
    ...
    if (empty($libelle)) {
        ...
        $qry = $db->prepare("insert into types_diplomes (priv_key, libelle) values (:pk, :l)");
...
switch ($mode) {
    case    "add":
        $PageTitle = "Ajout d'un type de diplôme";
        break;
    case    "chg":
        $PageTitle = "Modification du type de diplôme";
        break;
    case    "dlt":
        $PageTitle = "Suppression du type de diplôme";
        break;
    case    "dsp":
        $PageTitle = "Détail du type de diplôme";
        break;
    case    "lst":
        $PageTitle = "Liste des types de diplômes";
        break;
    default:
...
?>
    <form action="admin-types-diplomes.php" method="post" id="frm">
        ...
        <section class="fdb-block pt-0">
            <div class="container">
                ...
                <?php if ("lst" == $mode) {
                    $qry = $db->prepare("select * from types_diplomes order by libelle");
                    $qry->execute(array());
                    ?>
