...
if (("add" == $op) && $dsp) {
    ...
    if (empty($libelle)) {
        ...
        $qry = $db->prepare("insert into types_contrats (priv_key, libelle) values (:pk, :l)");
    ...
    $qry = $db->prepare("select * from types_contrats where code=:c");
    ...
    if (empty($libelle)) {
        ...
        if ((false !== ($record = $qry->fetch(PDO::FETCH_OBJ))) && checkVerifChecksum($verif, KEY_VERIF, $record->code, $record->priv_key)) {
            if ($libelle != $record->libelle) {
                $qry = $db->prepare("update types_contrats set libelle=:l where code=:c");
    ...
    $qry->execute(array(":c" => $code));
    if ((false !== ($record = $qry->fetch(PDO::FETCH_OBJ))) && checkVerifChecksum($verif, KEY_VERIF, $record->code, $record->priv_key)) {
        $qry = $db->prepare("delete from types_contrats where code=:c");
        $qry->execute(array(":c" => $code));
        $mode = "lst";
    } else {
        ajoute_erreur("Enregistrement inexistant, suppression impossible.");
...
switch ($mode) {
    case    "add":
        $PageTitle = "Ajout d'un type de contrat";
        break;
    case    "chg":
        $PageTitle = "Modification du type de contrat";
        break;
    case    "dlt":
        $PageTitle = "Suppression du type de contrat";
        break;
    case    "dsp":
        $PageTitle = "Détail du type de contrat";
        break;
    case    "lst":
        $PageTitle = "Liste des types de contrats";
        break;
    default:
...
?>
    <form action="admin-types-contrats.php" method="post" id="frm">
        ...
        <section class="fdb-block pt-0">
            <div class="container">
                ...
                <?php if ("lst" == $mode) {
                    $qry = $db->prepare("select * from types_contrats order by libelle");
                    $qry->execute(array());
                    ?>
                    ...
                    <div class="row justify-content-center pt-4">
                        <div class="col-12 col-md-7">
                            ...
                            if (("dsp" == $mode) || ("dlt" == $mode)) { ?>
                                <div class="row">
                                    <div class="col">
                                        Type de contrat : <?php print(htmlentities($libelle)); ?>
                                    </div>
                                </div>
                            <?php }
