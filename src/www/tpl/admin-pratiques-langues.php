...
if (("add" == $op) && $dsp) {
    ...
    if (empty($libelle)) {
        ...
        $qry = $db->prepare("insert into pratiques_langues (priv_key, libelle) values (:pk, :l)");
    ...
    $qry = $db->prepare("select * from pratiques_langues where code=:c");
    ...
    if (empty($libelle)) {
        ...
        if ((false !== ($record = $qry->fetch(PDO::FETCH_OBJ))) && checkVerifChecksum($verif, KEY_VERIF, $record->code, $record->priv_key)) {
            if ($libelle != $record->libelle) {
                $qry = $db->prepare("update pratiques_langues set libelle=:l where code=:c");
    ...
    $qry->execute(array(":c" => $code));
    if ((false !== ($record = $qry->fetch(PDO::FETCH_OBJ))) && checkVerifChecksum($verif, KEY_VERIF, $record->code, $record->priv_key)) {
        $qry = $db->prepare("delete from pratiques_langues where code=:c");
        $qry->execute(array(":c" => $code));
        $mode = "lst";
    } else {
        ajoute_erreur("Enregistrement inexistant, suppression impossible.");
...
switch ($mode) {
    case    "add":
        $PageTitle = "Ajout d'un pratique en langues";
        break;
    case    "chg":
        $PageTitle = "Modification du pratique en langues";
        break;
    case    "dlt":
        $PageTitle = "Suppression du pratique en langues";
        break;
    case    "dsp":
        $PageTitle = "Détail du pratique en langues";
        break;
    case    "lst":
        $PageTitle = "Liste des pratiques en langues";
        break;
    default:
...
?>
    <form action="admin-pratiques-langues.php" method="post" id="frm">
            ...
            <div class="container">
                ...
                <?php if ("lst" == $mode) {
                    ...
                    <table class="table text-center mt-5 d-table">
                        ...
                        <tr>
                            <th scope="col">ID</th>
                            <th scope="col">Pratique en langues</th>
                            <th scope="col">
                    ...
                    <div class="row justify-content-center pt-4">
                        <div class="col-12 col-md-7">
                            ...
                            if (("dsp" == $mode) || ("dlt" == $mode)) { ?>
                                <div class="row">
                                    <div class="col">
                                        Pratique en langues : <?php print(htmlentities($libelle)); ?>
                                    </div>
                                </div>
                            <?php }
                            if (("add" == $mode) || ("chg" == $mode)) { ?>
                                <div class="row">
                                    <div class="col">
                                        <input type="text" class="form-control" placeholder="Pratique en langues"
                                               id="libelle"
                                               name="libelle"
