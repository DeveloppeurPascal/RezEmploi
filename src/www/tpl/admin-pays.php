...
if (("add" == $op) && $dsp) {
    ...
    if (empty($libelle)) {
        ...
        $qry = $db->prepare("insert into pays (priv_key, libelle) values (:pk, :l)");
    ...
    $qry = $db->prepare("select * from pays where code=:c");
    ...
    if (empty($libelle)) {
        ...
        if ((false !== ($record = $qry->fetch(PDO::FETCH_OBJ))) && checkVerifChecksum($verif, KEY_VERIF, $record->code, $record->priv_key)) {
            if ($libelle != $record->libelle) {
                $qry = $db->prepare("update pays set libelle=:l where code=:c");
    ...
    $qry->execute(array(":c" => $code));
    if ((false !== ($record = $qry->fetch(PDO::FETCH_OBJ))) && checkVerifChecksum($verif, KEY_VERIF, $record->code, $record->priv_key)) {
        $qry = $db->prepare("delete from pays where code=:c");
        $qry->execute(array(":c" => $code));
        $mode = "lst";
    } else {
        ajoute_erreur("Enregistrement inexistant, suppression impossible.");
...
switch ($mode) {
    case    "add":
        $PageTitle = "Ajout d'un pays";
        break;
    case    "chg":
        $PageTitle = "Modification du pays";
        break;
    case    "dlt":
        $PageTitle = "Suppression du pays";
        break;
    case    "dsp":
        $PageTitle = "Détail du pays";
        break;
    case    "lst":
        $PageTitle = "Liste des pays";
        break;
    default:
...
?>
    <form action="admin-pays.php" method="post" id="frm">
        ...
        <section class="fdb-block pt-0">
            <div class="container">
                ...
                <?php if ("lst" == $mode) {
                    $qry = $db->prepare("select * from pays order by libelle");
                    $qry->execute(array());
                    ?>
                    ...
                    <div class="row justify-content-center pt-4">
                        <div class="col-12 col-md-7">
                            ...
                            if (("dsp" == $mode) || ("dlt" == $mode)) { ?>
                                <div class="row">
                                    <div class="col">
                                        Pays : <?php print(htmlentities($libelle)); ?>
                                    </div>
                                </div>
                            <?php }
                            if (("add" == $mode) || ("chg" == $mode)) { ?>
                                <div class="row">
                                    <div class="col">
                                        <input type="text" class="form-control" placeholder="Pays" id="libelle"
                                               name="libelle"
