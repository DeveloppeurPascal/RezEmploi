...
if (("add" == $op) && $dsp) {
    ...
    if (empty($libelle)) {
        ...
        $qry = $db->prepare("insert into niveaux_langues (priv_key, libelle) values (:pk, :l)");
    ...
    $qry = $db->prepare("select * from niveaux_langues where code=:c");
    ...
    if (empty($libelle)) {
        ...
        if ((false !== ($record = $qry->fetch(PDO::FETCH_OBJ))) && checkVerifChecksum($verif, KEY_VERIF, $record->code, $record->priv_key)) {
            if ($libelle != $record->libelle) {
                $qry = $db->prepare("update niveaux_langues set libelle=:l where code=:c");
    ...
    $qry->execute(array(":c" => $code));
    if ((false !== ($record = $qry->fetch(PDO::FETCH_OBJ))) && checkVerifChecksum($verif, KEY_VERIF, $record->code, $record->priv_key)) {
        $qry = $db->prepare("delete from niveaux_langues where code=:c");
        $qry->execute(array(":c" => $code));
        $mode = "lst";
    } else {
        ajoute_erreur("Enregistrement inexistant, suppression impossible.");
...
switch ($mode) {
    case    "add":
        $PageTitle = "Ajout d'un niveau en langues";
        break;
    case    "chg":
        $PageTitle = "Modification du niveau en langues";
        break;
    case    "dlt":
        $PageTitle = "Suppression du niveau en langues";
        break;
    case    "dsp":
        $PageTitle = "Détail du niveau en langues";
        break;
    case    "lst":
        $PageTitle = "Liste des niveaux en langues";
        break;
    default:
...
?>
    <form action="admin-niveaux-langues.php" method="post" id="frm">
