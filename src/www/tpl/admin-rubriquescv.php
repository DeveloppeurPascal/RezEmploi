...
if (("add" == $op) && $dsp) {
    ...
    if (empty($libelle)) {
        ...
        $qry = $db->prepare("insert into rubriquescv (priv_key, libelle) values (:pk, :l)");
    ...
    $qry = $db->prepare("select * from rubriquescv where code=:c");
    ...
    if (empty($libelle)) {
        ...
        if ((false !== ($record = $qry->fetch(PDO::FETCH_OBJ))) && checkVerifChecksum($verif, KEY_VERIF, $record->code, $record->priv_key)) {
            if ($libelle != $record->libelle) {
                $qry = $db->prepare("update rubriquescv set libelle=:l where code=:c");
    ...
    $qry->execute(array(":c" => $code));
    if ((false !== ($record = $qry->fetch(PDO::FETCH_OBJ))) && checkVerifChecksum($verif, KEY_VERIF, $record->code, $record->priv_key)) {
        $qry = $db->prepare("delete from rubriquescv where code=:c");
        $qry->execute(array(":c" => $code));
        $mode = "lst";
    } else {
        ajoute_erreur("Enregistrement inexistant, suppression impossible.");
...
switch ($mode) {
    case    "add":
        $PageTitle = "Ajout d'une rubrique des CV";
        break;
    case    "chg":
        $PageTitle = "Modification de la rubrique des CV";
        break;
    case    "dlt":
        $PageTitle = "Suppression de la rubrique des CV";
        break;
    case    "dsp":
        $PageTitle = "Détail de la rubrique des CV";
        break;
    case    "lst":
        $PageTitle = "Liste des rubriques des CV";
        break;
    default:
...
?>
    <form action="admin-rubriquescv.php" method="post" id="frm">
        ...
        <section class="fdb-block pt-0">
            <div class="container">
                ...
                <?php if ("lst" == $mode) {
                    $qry = $db->prepare("select * from rubriquescv order by libelle");
                    $qry->execute(array());
                    ?>
                    <table class="table text-center mt-5 d-table">
                        ...
                        <tr>
                            <th scope="col">ID</th>
                            <th scope="col">Rubrique des CV</th>
                            <th scope="col">
                    ...
                    <div class="row justify-content-center pt-4">
                        <div class="col-12 col-md-7">
                            ...
                            if (("dsp" == $mode) || ("dlt" == $mode)) { ?>
                                <div class="row">
                                    <div class="col">
                                        Rubrique des CV : <?php print(htmlentities($libelle)); ?>
                                    </div>
                                </div>
                            <?php }
                            if (("add" == $mode) || ("chg" == $mode)) { ?>
                                <div class="row">
                                    <div class="col">
                                        <input type="text" class="form-control" placeholder="Rubrique des CV"
                                               id="libelle"
                                               name="libelle"
